...
def calculate_efficiencies():
    ...
    canvas_npt = FigureCanvasTkAgg(fig_npt, master=root)
    canvas_npt.draw()
    canvas_npt.get_tk_widget().grid(column=0, row=6, columnspan=2, padx=10, pady=10)
...
def reset():
    ...
    for widget in root.grid_slaves():
        if int(widget.grid_info()["row"]) in [5, 6]:
            widget.grid_forget()
